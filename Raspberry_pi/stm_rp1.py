...
DEBUG_PRINT_PERIOD_SEC = 1.0  # 1초마다 R/G/B 카운트를 터미널에도 출력 (튜닝용)
...
def gen_frames():
    ...
    while True:
        ...
        # ✅ 변경: 신호 매핑을 RED(정지)/GREEN(저속)/BLUE(정속)로 바꿈.
        #    LED가 노란색을 만들 때는 R+G를 섞은 혼합색이라 채도가 약하고
        #    카메라에서 색조(H)가 흔들려 인식률이 낮음. 파란색은 원색이라
        #    채도/색조가 안정적으로 잡혀 구분이 훨씬 쉬움.

        # 초록색 범위 (-> 저속 SLOW)
        # S(채도)/V(명도) 하한을 50 -> 120으로 상향.
        # LED는 자체발광이라 채도/명도가 매우 높음. 나무책상/바닥처럼
        # 칙칙한 색(채도/명도 낮음)은 장소가 바뀌어도 이 기준에 안 걸리게 됨.
        lower_green = np.array([35, 120, 120])
        upper_green = np.array([85, 255, 255])

        # 파란색 범위 (-> 정속 NORMAL)  ※ 기존 노란색 자리를 대체
        lower_blue = np.array([100, 120, 120])
        upper_blue = np.array([130, 255, 255])

        # 빨간색 범위 (HSV 구조상 0 부근과 180 부근 두 영역으로 나뉨, -> 정지 STOP)
        ...
        counts = sorted(
            [(cnt_red, 0, "RED (0) -> STOP"),
             (cnt_green, 1, "GREEN (1) -> SLOW"),
             (cnt_blue, 2, "BLUE (2) -> NORMAL")],
            reverse=True
        )
        ...
        # ✅ 1초마다 터미널에도 R/G/B 카운트 출력 (인식 안 될 때도 항상 보임)
        now_dbg = time.time()
        if now_dbg - _last_debug_print >= DEBUG_PRINT_PERIOD_SEC:
            _last_debug_print = now_dbg
            print(f"[DEBUG] R:{cnt_red} G:{cnt_green} B:{cnt_blue} "
        ...
        # 항상 R/G/B 픽셀 카운트를 화면에 표시 (튜닝용 디버그).
        # 어느 조건(threshold 미달 vs dominance 미달)에서 막히는지 바로 확인 가능.
        debug_line = f"R:{cnt_red} G:{cnt_green} B:{cnt_blue} (th={threshold})"
